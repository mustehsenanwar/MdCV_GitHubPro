...
class KTTheme:
    ...
    # Get an static path in static folder by path
    def asset(path):
        return static(path)
    ...
    def includeFavicon():
        # Use static() to use builtin function for static folder
        # Refer in core/settings.py for STATIC_URL settings
    ...
    def setLayout(view, context = {}):
        ...
        # Get module path
        module = 'templates.{}._bootstrap.{}'.format(settings.KT_THEME_LAYOUT_DIR.replace('/', '.'), layout)
        ...
        if not util.find_spec(module) == None:
            ...
            module = 'templates.{}._bootstrap.default'.format(settings.KT_THEME_LAYOUT_DIR.replace('/', '.'))
    ...
    def getIcon(name, class_name='', type=''):
        ...
        if type == 'duotone':
            ...
            if not icons:
                with open(settings.BASE_DIR / 'core/libs/icons.json') as f:
                    icons = json.load(f)
